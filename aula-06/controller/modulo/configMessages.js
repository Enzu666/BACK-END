/*********************************************************************************
 * Objetivo: Arquivo responsável pela configuração e padronização das mensagens
 *              da API
 * Data: 17/04/26 
 * Autor: Enzo
 * Versão: 1.0
 *********************************************************************************/

//Padronização de cabelho para o retorno dos endpoints da API
const DEFAULT_MESSAGE = {
    api_description:    'API para gerenciar controle de filmes',
    desenvolvedor:      'Enzo',
    version:            "1.0.4.26",
    status: Boolean,
    status_code: Number,
    response: {}
}

//Mensagem de erro da API
const ERROR_BAD_REQUEST = {status: false, status_code:400, message: 'Os dados enviados na requisição não esta correto.'}

//Mensagens de sucesso da API
const SUCESS_CREATED_ITEM = {status: true, status_code: 201, message:"Registro inserido com sucesso."}

module.exports = {
    DEFAULT_MESSAGE,
    ERROR_BAD_REQUEST,
    SUCESS_CREATED_ITEM
}